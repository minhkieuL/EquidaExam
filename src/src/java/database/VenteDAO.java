    ...
    */
    public static ArrayList<Client>  getLesClients(Connection connection, String codeCateg){      
        ...
        try
        {
            //preparation de la requete     
            //codeCateg="ETE";
            requete=connection.prepareStatement("SELECT c.*, p.nom AS nomPays, cv.libelle FROM client c, pays p, clientcategvente cc, categVente cv, utilisateur u WHERE u.id = c.id AND u.codePays=p.code AND cc.codeClient=c.id AND cv.code=cc.codeCategVente AND codeCategVente= ? ");
